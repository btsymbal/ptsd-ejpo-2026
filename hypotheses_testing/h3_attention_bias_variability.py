...
#
# **Hypothesis**: Participants in the PTSD group will show higher across-slide
# variability of the attention bias score (delta between threat and paired baseline
# dwell time percentage) than the no-PTSD group. Higher delta variability indicates
# inconsistent attentional bias — fluctuating between vigilance and avoidance of
# threat across presentations.
#
# **Method**: Two-tailed group comparisons (PTSD vs no-PTSD) on within-participant
# SD of delta dwell time percentage for 4 threat categories. Test selection follows
...
DV_COLS = [f'std_delta_dwell_pct_{cat}' for cat in THREAT_CATEGORIES]
ALPHA = 0.05

FIG_DIR = 'figures/h3_attention_bias_variability'
...
print(f"\nDelta dwell variability DV columns: {DV_COLS}")
...
# %%
print("\n" + "=" * 90)
print("RESULTS SUMMARY — H3: Delta Dwell Time Variability × PTSD Group")
print("=" * 90)
...
print(f"\nOverall: H3 {'supported' if any_sig else 'NOT supported'} at α = {ALPHA} (BH-corrected)")
print("=" * 90)
...
plot_long = plot_df.melt(id_vars=['Group'], value_vars=DV_COLS,
                          var_name='Category', value_name='SD Delta Dwell %')
plot_long['Category'] = plot_long['Category'].str.replace('std_delta_dwell_pct_', '', regex=False)

fig, ax = plt.subplots(figsize=(10, 6))
sns.violinplot(data=plot_long, x='Category', y='SD Delta Dwell %', hue='Group',
               split=True, inner=None, palette={'PTSD': '#d9534f', 'No-PTSD': '#5bc0de'},
               alpha=0.4, ax=ax)
sns.stripplot(data=plot_long, x='Category', y='SD Delta Dwell %', hue='Group',
...
ax.set_title('SD of Delta Dwell % on Threat Stimuli by PTSD Group')
ax.set_xlabel('Threat Category')
ax.set_ylabel('SD of Delta Dwell Time (%)')

fig.savefig(f'{FIG_DIR}/violin_delta_dwell_variability_by_group.png', dpi=600, bbox_inches='tight')
plt.close(fig)
print(f'Saved: {FIG_DIR}/violin_delta_dwell_variability_by_group.png')
...
ax.set_title('Forest Plot — Effect Sizes for Delta Dwell Variability (PTSD vs No-PTSD)')
ax.invert_yaxis()
...
# %% [markdown]
# ## 9. Visualization 3 — Bar Chart (Group Means, 95% CI)
...
ax.set_xlabel('Threat Category')
ax.set_ylabel('SD of Delta Dwell Time (%) (95% CI)')
ax.set_title('SD of Delta Dwell % on Threat Stimuli by Group')
ax.legend(title='Group')

fig.savefig(f'{FIG_DIR}/bar_delta_dwell_variability_by_group.png', dpi=600, bbox_inches='tight')
plt.close(fig)
print(f'Saved: {FIG_DIR}/bar_delta_dwell_variability_by_group.png')
