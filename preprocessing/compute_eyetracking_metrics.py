...
# -------------------------------------------------------------------
def compute_slide_dwell_pct(slide_df, slide_num):
    """Return {img_id: dwell_pct} for both images on this slide.

    dwell_pct = (time spent gazing at image / expected_duration_ms) * 100
...
def compute_blinks(session_df):
    ...
    Returns a dict with:
      - total_blink_count
      - blink_count_{cat}
      - mean/std_blink_interval_ms
      - mean_blink_latency_{cat}
    ...
    for cat in CATEGORIES:
        ...
        result[f'blink_count_{cat}'] = len(durs)
    ...
    # Blink intervals (time between consecutive blink onsets)
    if len(blinks) > 1:
        onsets = [b[0] for b in blinks]
        intervals = np.diff(onsets)
        result['mean_blink_interval_ms'] = np.mean(intervals)
        result['std_blink_interval_ms'] = np.std(intervals, ddof=1) if len(intervals) > 1 else np.nan
    else:
        result['mean_blink_interval_ms'] = np.nan
        result['std_blink_interval_ms'] = np.nan
    ...
    for cat in CATEGORIES:
        ...
        result[f'mean_blink_latency_{cat}'] = np.mean(lats) if lats else np.nan
...
for fpath in session_files:
    ...
    df = pd.read_csv(fpath, usecols=['TIMESTAMP', 'BLINK', 'SCENE_INDEX', 'IMAGE'])
...
dwell_cols = [c for c in metrics_df.columns if 'dwell_pct' in c and c.startswith('mean_')]
...
report_lines += [
    ...
    f"| blink_count | {len(CATEGORIES)} |",
    f"| mean_blink_interval_ms | 1 |",
    f"| std_blink_interval_ms | 1 |",
    f"| mean_blink_latency | {len(CATEGORIES)} |",
